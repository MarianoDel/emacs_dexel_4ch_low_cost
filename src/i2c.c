...
void I2C1_Init (void)
{
    ...
    // // PA9 SCL; PA10 SDA -> AF6
    // unsigned int temp = 0;
    // temp = GPIOA->AFR[1];
    // temp &= 0xFFFFF00F;
    // temp |= 0x00000660;    //PA10 -> AF6 PA9 -> AF6
    // GPIOA->AFR[1] = temp;
    ...
    temp = GPIOB->AFR[0];
    temp &= 0x0FFFFFFF;
    temp |= 0x60000000;    //PB7 -> AF6
    GPIOB->AFR[0] = temp;

    temp = GPIOB->AFR[1];
    temp &= 0xFFFFFFF0;
    temp |= 0x00000006;    //PB8 -> AF6
    GPIOB->AFR[1] = temp;
...
volatile unsigned char i2c1_bytes_left = 0;
volatile unsigned char i2c1_stopped = 0;
volatile unsigned char * p_i2c1_data = 0;
